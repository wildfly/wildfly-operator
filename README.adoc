...
An example of a custom resource of `WildFlyServer` is described in https://github.com/wildfly/wildfly-operator/blob/master/deploy/cr/wildfly-operator-quickstart-cr.yaml[wildfly-operator-quickstart-cr.yaml]:
...
kind: WildFlyServer
metadata:
  name: wildfly-operator-quickstart
spec:
...
[source,shell]
----
$ kubectl create -f deploy/cr/quickstart-cr.yaml
...
After installing the `WildFlyServer` resource from `deploy/cr/quickstart-cr.yaml`, you have to create a route to expose it from OpenShift:
