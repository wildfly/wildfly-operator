...
After installing the `WildFlyServer` resource from `config/samples/quickstart-cr.yaml`, you have to create a route to expose it from OpenShift:
...
The Operator will be deployed as a local application. The test suite creates the resources for each test under the
`wildfly-op-test-ns` namespace:
...
If you stop the execution or if a test fails, you should clean any resource left in your cluster before running the tests again:
...
You need to have the latest Operator image available somewhere. Before running the e2e tests, build and push it to a public
container registry:
...
Then run the tests specifying that your Operator will use that image:

[source,shell]
----
$ make test IMG="quay.io/my-quay-user/wildfly-operator-dev:latest"
----

You can monitor the resources in a different terminal by using:
...
applied. The following instructions describes how to prepare a developer image, bundle and catalog and how to
install the Operator in an OpenShift cluster that has OLM installed. The example uses `quay.io` as container registry:
...
IMG="quay.io/your-username/wildfly-operator-dev:latest" \
BUNDLE_IMG="quay.io/your-username/wildfly-operator-bundle:1.0.0" \
CATALOG_IMG="quay.io/your-username/wildfly-operator-catalog:1.0.0"
----

* `manifests docker-build docker-push`: Will create your Operator image and push it to your registry.
...
Then install a CatalogSource which contains information for accessing a repository of Operators.
The catalog source needs to be created on the openshift-marketplace namespace:
...
Then you should be ready to install the Operator from the OpenShift web console or, for example, by directly adding a subscription
