...
metadata:
  name: taskrs-app
spec:
  applicationImage: "quay.io/wildfly-quickstarts/taskrs-app:17.0"
  replicas: 2
  env:
...
This custom resource will run the https://github.com/jfdenise/s2i-wildfly/tree/master/17.0/test/test-app-postgres[test-app-postgres] application from the https://quay.io/repository/wildfly-quickstarts/taskrs-app[quay.io/wildfly-quickstarts/taskrs-app:17.0] image that is using the https://github.com/wildfly/wildfly-s2i[Source-to-Image (S2I) template for WildFly].
...
Spec:
  Application Image:  quay.io/wildfly-quickstarts/taskrs-app:17.0
  Env:
    Name:   POSTGRESQL_SERVICE_HOST
    Value:  postgresql
    Name:   POSTGRESQL_SERVICE_PORT
