...
func (r *WildFlyServerReconciler) checkRecovery(reqLogger logr.Logger, scaleDownPod *corev1.Pod, w *wildflyv1alpha1.WildFlyServer) (checkResult, string, error) {
	...
	if scaleDownPod.Annotations[markerRecoveryPort] == "" {
		reqLogger.Info("Verification that the transaction recovery listener of the pod is ready to recover transactions", "Pod name", scaleDownPodName)
		...
		if txrecoverydefined, err := strconv.ParseBool(isTxRecoveryListenerEnabledAsString); err == nil && !txrecoverydefined {
			reqLogger.Info("The transaction recovery listener of the pod is not enabled", "Pod name", scaleDownPodName)
		...
		reqLogger.Info("Query the app server to find out the transaction recovery port of the pod", "Pod name", scaleDownPodName)
		...
		if err != nil {
			reqLogger.Error(err, "Error reading the transaction recovery port", "Pod name", scaleDownPodName, "Default port", strconv.Itoa(int(scaleDownPodRecoveryPort)))
		}
		...
		if err != nil {
			...
			if errUpdate := resources.Patch(w, r.Client, scaleDownPod, patch); errUpdate != nil {
				reqLogger.Info("Cannot update the pod while resetting the recovery port annotation",
					"Pod name", scaleDownPodName, "Annotations", scaleDownPod.Annotations, "Error", errUpdate)
			}
	...
	reqLogger.Info("Executing the recovery scan for the pod", "Pod name", scaleDownPodName, "Pod IP", scaleDownPodIP, "Recovery port", scaleDownPodRecoveryPort)
		...
		go func() {
			...
			if wildflyServerSpecPodStatus.State != wildflyv1alpha1.PodStateScalingDownClean {
				...
				if outcome == clean {
					...
					reqLogger.Info("The pod is trying to recover unfinished transactions", "Pod name", corePodTemp.ObjectMeta.Name, "Message", message)
					...
					reqLogger.Info("The recovery counter of the pod is: "+strconv.Itoa(int(wildflyServerSpecPodStatus.RecoveryCounter)), "Pod name", corePodTemp.ObjectMeta.Name)
					...
					reqLogger.Info("The pod has heuristic transactions", "Pod name", corePodTemp.ObjectMeta.Name, "Message", message)
